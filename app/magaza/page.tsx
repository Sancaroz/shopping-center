...
export default function CatalogPage() {
  ...
  return <main className="catalog-page">
    ...
    <div className="catalog-body">
      <aside className="catalog-filters"><p>{market==="TR"?"KATEGORİLER":"CATEGORIES"}</p><button className={category===null?"active":""} onClick={()=>setCategory(null)}>{market==="TR"?"Tüm ürünler":"All products"} <span>{products.filter(p=>market==="TR"?p.marketTr:p.marketGlobal).length}</span></button>{roots.map(root=><div key={root.id}><button className={category===root.id?"active":""} onClick={()=>setCategory(root.id)}>{categoryName(root)}</button>{categories.filter(item=>item.parentId===root.id).map(child=><button className={`sub ${category===child.id?"active":""}`} key={child.id} onClick={()=>setCategory(child.id)}>— {categoryName(child)}</button>)}</div>)}</aside>
