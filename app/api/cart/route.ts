...
export async function GET(request:Request){
  try{
    ...
    if(!cart)return response({items:[],market:"TR"},token,false,200,request);
    ...
    return response({items,market:cart.market},token,false,200,request);
  }catch{return Response.json({items:[],market:"TR"},{headers:{"Cache-Control":"no-store"}});}
}
...
export async function POST(request:Request){
  ...
  if(!session.cart&&quantity>maximum)return response({error:`En fazla ${maximum} adet ekleyebilirsiniz.`,maximum},session.token,false,409,request);
  ...
  if(!cart){token=crypto.randomUUID();[cart]=await db.insert(carts).values({token,market}).returning();created=true;}
  const condition=and(eq(cartItems.cartId,cart.id),eq(cartItems.productId,productId),variantId?eq(cartItems.variantId,variantId):isNull(cartItems.variantId));const[existing]=await db.select().from(cartItems).where(condition).limit(1);const next=(existing?.quantity??0)+quantity;if(next>maximum)return response({error:`En fazla ${maximum} adet ekleyebilirsiniz.`,maximum},token,created,409,request);
  if(existing)await db.update(cartItems).set({quantity:next}).where(eq(cartItems.id,existing.id));else await db.insert(cartItems).values({cartId:cart.id,productId,variantId,quantity});await db.update(carts).set({market,updatedAt:new Date().toISOString()}).where(eq(carts.id,cart.id));return response({ok:true,quantity:next},token,created,201,request);
}
