...
export async function POST(request:Request) {
  ...
  if(duplicate){if(duplicate.email!==email)return Response.json({error:"Bu sipariş isteği doğrulanamadı."},{status:409});if(duplicate.creationState!=="ready")return Response.json({error:"Sipariş talebiniz işleniyor. Lütfen birkaç saniye sonra tekrar deneyin."},{status:409,headers:{"Retry-After":"2","Cache-Control":"no-store"}});return Response.json({orderNumber:duplicate.orderNumber,subtotal:duplicate.subtotal,discountAmount:duplicate.discountAmount,shippingAmount:duplicate.shippingAmount,total:duplicate.total,market:duplicate.market},{status:200});}
  ...
  let order:typeof orders.$inferSelect|undefined;try{[order] = await db.insert(orders).values({
    ...
    note, subtotal,shippingAmount,total,discountAmount,promotionId:promotionResult.promotion?.id??null,promoCode:promotionResult.promotion?.code??"",requestKey,creationState:"creating",privacyConsentAt:consentAt,termsConsentAt:consentAt,termsVersion:contractSnapshot.version,termsSnapshotJson:contractSnapshot.json,termsSnapshotHash:contractSnapshot.hash,inventoryApplied:true,reservationState:"active",reservationExpiresAt:verificationExpiresAt,verificationTokenHash,verificationExpiresAt,billingType,billingName,billingAddress,billingCity,billingPostalCode:billingPostalCode??"",billingCountry,billingTaxOffice:billingTaxOffice??"",billingTaxNumber:billingTaxNumber??"",pricingTaxStatus:settings.taxDisplayMode??"pending",sellerSnapshotJson,
  }).returning();
