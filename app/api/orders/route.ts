import { and, asc, desc, eq, exists, gt, gte, isNull, lt, lte, or, sql } from "drizzle-orm";
import { getDb } from "../../../db";
import { cartItems, carts, fulfillmentChecklists, inventoryMovements, inventoryOperations, notificationOutbox, orderItems, orders, products, promotionRedemptions, promotions, productVariants, shipmentEvents, storeSettings } from "../../../db/schema";
...
import {recoverStaleCreatingOrder} from "../../order-creation-recovery";
...
async function queueNotification(order:typeof orders.$inferSelect,event:NotificationEvent,verificationUrl="",status="draft"){const message=buildOrderNotification(order,event,verificationUrl);return getDb().insert(notificationOutbox).values({orderId:order.id,eventKey:`${order.id}:${event}`,eventType:event,recipient:message.recipient,subject:message.subject,body:message.body,status}).onConflictDoNothing({target:notificationOutbox.eventKey}).returning({id:notificationOutbox.id});}
...
export async function POST(request:Request) {
  ...
  const requestKey=String(body.requestKey??"");const cartRevision=boundedText(body.cartRevision,100);
  ...
  if (!customerName || customerName.length<2 || !isValidEmail(email) || !phone || !isValidPhone(phone) || !address || address.length<5 || !city || city.length<2 || !country || postalCode===null || note===null || !cartRevision || body.privacyConsent!==true || body.termsConsent!==true || !isValidRequestKey(requestKey)) return Response.json({ error:"Lütfen zorunlu teslimat ve onay bilgilerini eksiksiz girin." }, { status:400 });
  ...
  if(duplicate){if(duplicate.email!==email)return Response.json({error:"Bu sipariş isteği doğrulanamadı."},{status:409});if(duplicate.creationState==="ready")return Response.json({orderNumber:duplicate.orderNumber,subtotal:duplicate.subtotal,discountAmount:duplicate.discountAmount,shippingAmount:duplicate.shippingAmount,total:duplicate.total,market:duplicate.market},{status:200});const staleCutoff=new Date(Date.now()-2*60_000).toISOString();if(!["creating","recovering"].includes(duplicate.creationState)||duplicate.updatedAt>staleCutoff||!await recoverStaleCreatingOrder(db,duplicate,staleCutoff))return Response.json({error:"Sipariş talebiniz işleniyor. Lütfen birkaç saniye sonra tekrar deneyin."},{status:409,headers:{"Retry-After":"2","Cache-Control":"no-store"}});}
  ...
  if(cart.revision!==cartRevision)return Response.json({error:"Çantanız teslimat sayfası açıldıktan sonra değişti. Güncel özeti kontrol edip tekrar deneyin.",code:"cart_changed"},{status:409,headers:{"Cache-Control":"no-store"}});
  ...
  const[currentCart]=await db.select({revision:carts.revision}).from(carts).where(eq(carts.id,cart.id)).limit(1);if(!currentCart||currentCart.revision!==cartRevision)return Response.json({error:"Çantanız sipariş hazırlanırken değişti. Güncel özeti kontrol edip tekrar deneyin.",code:"cart_changed"},{status:409,headers:{"Cache-Control":"no-store"}});
  ...
  let order:typeof orders.$inferSelect|undefined;try{const orderInsert=db.insert(orders).values({
    orderNumber, market:cart.market, customerName, email, phone, address, city,
    postalCode, country:quote.country,
    note, subtotal,shippingAmount,total,discountAmount,promotionId:promotionResult.promotion?.id??null,promotionClaimState:promotionResult.promotion?"pending":"none",promoCode:promotionResult.promotion?.code??"",requestKey,creationState:"creating",privacyConsentAt:consentAt,termsConsentAt:consentAt,termsVersion:contractSnapshot.version,termsSnapshotJson:contractSnapshot.json,termsSnapshotHash:contractSnapshot.hash,inventoryApplied:true,inventoryOperationKey:reservation.operationKey,reservationState:"active",reservationExpiresAt:verificationExpiresAt,verificationTokenHash,verificationExpiresAt,billingType,billingName,billingAddress,billingCity,billingPostalCode:billingPostalCode??"",billingCountry,billingTaxOffice:billingTaxOffice??"",billingTaxNumber:billingTaxNumber??"",pricingTaxStatus:settings.taxDisplayMode??"pending",sellerSnapshotJson,
  }).returning();
  if(promotionResult.promotion){const promo=promotionResult.promotion;const nowIso=new Date().toISOString();const promotionOperationKey=`promotion-claim:${requestKey}`;const claimConditions=[eq(promotions.id,promo.id),eq(promotions.active,true),or(eq(promotions.market,"BOTH"),eq(promotions.market,cart.market==="GLOBAL"?"GLOBAL":"TR"))!,or(isNull(promotions.startsAt),lte(promotions.startsAt,nowIso))!,or(isNull(promotions.endsAt),gte(promotions.endsAt,nowIso))!];if(promo.usageLimit>0)claimConditions.push(lt(promotions.usedCount,promo.usageLimit));const promotionClaim=db.update(promotions).set({usedCount:sql`${promotions.usedCount}+1`,lastClaimOperationKey:promotionOperationKey,updatedAt:nowIso}).where(and(...claimConditions));const claimRecorded=exists(db.select({id:promotions.id}).from(promotions).where(and(eq(promotions.id,promo.id),eq(promotions.lastClaimOperationKey,promotionOperationKey))));const orderClaim=db.update(orders).set({promotionClaimState:"active",updatedAt:nowIso}).where(and(eq(orders.requestKey,requestKey),eq(orders.creationState,"creating"),claimRecorded));const orderClaimRecorded=exists(db.select({id:orders.id}).from(orders).where(and(eq(orders.requestKey,requestKey),eq(orders.promotionClaimState,"active"))));const operationInsert=db.insert(inventoryOperations).values({operationKey:sql<string>`CASE WHEN ${and(claimRecorded,orderClaimRecorded)} THEN ${promotionOperationKey} ELSE NULL END`,kind:"promotion_claim",state:"committed"}).returning({operationKey:inventoryOperations.operationKey});const results=await db.batch([orderInsert,promotionClaim,orderClaim,operationInsert]);order=results[0][0];const operation=results[3][0];if(!order||!operation)throw new Error("promotion claim failed");}
  else [order]=await orderInsert;
  ...
  await queueNotification(order,"verification",`https://mysa-objets-store.robologai.chatgpt.site/siparis-dogrula?token=${verificationToken}`,"held");
  await queueNotification(order,"received","","held");
  const heldVerification=exists(db.select({id:notificationOutbox.id}).from(notificationOutbox).where(and(eq(notificationOutbox.eventKey,`${order.id}:verification`),eq(notificationOutbox.status,"held"))));const heldReceived=exists(db.select({id:notificationOutbox.id}).from(notificationOutbox).where(and(eq(notificationOutbox.eventKey,`${order.id}:received`),eq(notificationOutbox.status,"held"))));const activeReservation=exists(db.select({operationKey:inventoryOperations.operationKey}).from(inventoryOperations).where(and(eq(inventoryOperations.operationKey,reservation.operationKey),eq(inventoryOperations.state,"active"))));const readyGuard=exists(db.select({id:orders.id}).from(orders).where(and(eq(orders.id,order.id),eq(orders.creationState,"ready"))));const draftVerification=exists(db.select({id:notificationOutbox.id}).from(notificationOutbox).where(and(eq(notificationOutbox.eventKey,`${order.id}:verification`),eq(notificationOutbox.status,"draft"))));const draftReceived=exists(db.select({id:notificationOutbox.id}).from(notificationOutbox).where(and(eq(notificationOutbox.eventKey,`${order.id}:received`),eq(notificationOutbox.status,"draft"))));const finalization=await db.batch([db.update(orders).set({creationState:"ready",updatedAt:new Date().toISOString()}).where(and(eq(orders.id,order.id),eq(orders.creationState,"creating"),heldVerification,heldReceived,activeReservation)).returning({id:orders.id}),db.update(notificationOutbox).set({status:"draft",updatedAt:new Date().toISOString()}).where(and(eq(notificationOutbox.orderId,order.id),eq(notificationOutbox.status,"held"),readyGuard)).returning({id:notificationOutbox.id}),db.update(inventoryOperations).set({state:"committed",updatedAt:new Date().toISOString()}).where(and(eq(inventoryOperations.operationKey,reservation.operationKey),eq(inventoryOperations.state,"active"),readyGuard,draftVerification,draftReceived)).returning({operationKey:inventoryOperations.operationKey})]);const completed=finalization[0][0];const releasedNotifications=finalization[1];const committedReservation=finalization[2][0];if(!completed||releasedNotifications.length!==2||!committedReservation)throw new Error("order finalization failed");}catch{const rolledBack=await reservation.rollback();const[inventoryState]=await db.select({state:inventoryOperations.state}).from(inventoryOperations).where(eq(inventoryOperations.operationKey,reservation.operationKey)).limit(1);const inventoryRecovered=rolledBack||inventoryState?.state==="rolled_back";let promotionRecovered=true;if(order?.promotionId){await releasePromotionClaim(db,{orderId:order.id,promotionId:order.promotionId});const[current]=await db.select({state:orders.promotionClaimState}).from(orders).where(eq(orders.id,order.id)).limit(1);promotionRecovered=!current||current.state!=="active";}if(order?.id&&inventoryRecovered&&promotionRecovered)await db.delete(orders).where(and(eq(orders.id,order.id),eq(orders.creationState,"creating")));const[retry]=await db.select().from(orders).where(eq(orders.requestKey,requestKey)).limit(1);if(retry){if(retry.email!==email)return Response.json({error:"Bu sipariş isteği doğrulanamadı."},{status:409});if(retry.creationState!=="ready")return Response.json({error:"Sipariş talebiniz işleniyor. Lütfen birkaç saniye sonra tekrar deneyin."},{status:409,headers:{"Retry-After":"2","Cache-Control":"no-store"}});return Response.json({orderNumber:retry.orderNumber,subtotal:retry.subtotal,discountAmount:retry.discountAmount,shippingAmount:retry.shippingAmount,total:retry.total,market:retry.market},{status:200});}return Response.json({error:"Sipariş talebi kaydedilemedi; ayrılan stok geri bırakıldı."},{status:500});}
  ...
  try{const exactDeletes=priced.map(line=>db.delete(cartItems).where(and(eq(cartItems.id,line.cartItemId),eq(cartItems.cartId,cart.id),eq(cartItems.quantity,line.quantity))));const excessReductions=priced.map(line=>db.update(cartItems).set({quantity:sql`${cartItems.quantity}-${line.quantity}`}).where(and(eq(cartItems.id,line.cartItemId),eq(cartItems.cartId,cart.id),gt(cartItems.quantity,line.quantity))));await db.batch([...exactDeletes,...excessReductions,db.update(carts).set({revision:crypto.randomUUID(),updatedAt:new Date().toISOString()}).where(eq(carts.id,cart.id))]);}catch{}
...
export async function PATCH(request:Request) {
  ...
  let claimedOrder:typeof orders.$inferSelect|undefined;
  const cancellationReleasesInventory=nextStatus==="cancelled"&&["active","committed"].includes(existing.reservationState);
  if(cancellationReleasesInventory){const released=await releaseOrderReservation(db,orderId,"released",true,{expectedStatus:existing.status,expectedUpdatedAt:existing.updatedAt,releasePromotion,orderUpdates:updates});if(released)[claimedOrder]=await db.select().from(orders).where(eq(orders.id,orderId)).limit(1);}
  else [claimedOrder]=await db.update(orders).set(updates).where(and(eq(orders.id,orderId),eq(orders.status,existing.status),eq(orders.updatedAt,existing.updatedAt))).returning();
  ...
  if(releasePromotion&&existing.promotionId&&!cancellationReleasesInventory)await releasePromotionClaim(db,{orderId,promotionId:existing.promotionId});
