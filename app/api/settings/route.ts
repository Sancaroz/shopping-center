...
export async function PUT(request: Request) { if (!(await getChatGPTUser())) return Response.json({ error: "Yetkisiz erişim" }, { status: 401 }); const body = await request.json() as Record<string, unknown>; const db = getDb(); const allowed = Object.keys(defaults) as (keyof typeof defaults)[]; const rows=await db.select().from(storeSettings);const current=Object.fromEntries(rows.map(row=>[row.key,row.value]));const values=Object.fromEntries(allowed.map(key=>[key,String(body[key]??current[key]??defaults[key])]));if(values.legalStatus==="complete"){const required=[["Ticari unvan",values.legalName],["Şirket türü",values.legalBusinessType],["Merkez adresi",values.legalAddress],["Vergi dairesi",values.legalTaxOffice],["Vergi numarası",values.legalTaxNumber],["Hukuki e-posta",values.legalEmail],["Telefon",values.legalPhone],["İade adresi",values.returnAddress],["Ön bilgilendirme",values.preliminaryInformationTr],["Mesafeli satış sözleşmesi",values.distanceSalesTermsTr]];const missing=required.filter(([,value])=>!value.trim()).map(([label])=>label);if(missing.length)return Response.json({error:`Yayına hazır durumu için eksik alanlar: ${missing.join(", ")}.`},{status:409});}if(values.salesMode==="live"){const blockers:string[]=["ödeme sağlayıcısı teknik entegrasyonu"];if(values.legalStatus!=="complete")blockers.push("şirket ve hukuki bilgiler");if(values.paymentProviderStatus!=="active"||!values.paymentProviderName.trim())blockers.push("aktif ödeme sağlayıcısı");if(values.etbisStatus!=="complete")blockers.push("ETBİS kaydı");if(!values.returnCarrier.trim())blockers.push("anlaşmalı iade kargosu");if(values.preliminaryInformationTr.startsWith("TASLAK")||values.distanceSalesTermsTr.startsWith("TASLAK"))blockers.push("onaylı sözleşme metinleri");const activeProducts=await db.select({id:products.id}).from(products).where(eq(products.active,true)).limit(1);if(!activeProducts.length)blockers.push("yayındaki ürün");if(blockers.length)return Response.json({error:`Canlı satış modu için tamamlanmalı: ${blockers.join(", ")}.`},{status:409});}await db.batch(allowed.map(key => db.insert(storeSettings).values({ key, value: values[key], updatedAt: new Date().toISOString() }).onConflictDoUpdate({ target: storeSettings.key, set: { value: values[key], updatedAt: new Date().toISOString() } }))); return Response.json({ settings: values }); }
