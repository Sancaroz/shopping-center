...
import { contactMessages, notificationOutbox, orders, products, productVariants, returnRequests } from "../../../db/schema";
...
export async function GET() {
  ...
  const db=getDb();
  const [orderRows,productRows,variantRows,returnRows,messageRows,notificationRows]=await Promise.all([
  ...
  const alerts:Alert[]=[
    ...
    ...openMessages.filter(item=>item.status==="new"&&hours(item.createdAt)>=24).map(item=>({key:`message-${item.id}`,level:"warning" as const,title:`${item.name} adlı müşterinin mesajı bekliyor`,detail:`${item.subject} · ${Math.floor(hours(item.createdAt))} saat`,href:"/admin",createdAt:item.createdAt})),
  ].sort((a,b)=>(a.level==="urgent"?0:1)-(b.level==="urgent"?0:1)||(a.createdAt??"").localeCompare(b.createdAt??""));
  return Response.json({
    generatedAt:new Date().toISOString(),
    metrics:{activeOrders:activeOrders.length,newOrders:activeOrders.filter(order=>order.status==="new").length,preparingOrders:activeOrders.filter(order=>order.status==="preparing").length,shippingExceptions:activeOrders.filter(order=>order.deliveryStatus==="exception").length,staleShipments:activeOrders.filter(order=>order.status==="shipped"&&hours(order.lastShipmentEventAt??order.shippedAt??order.updatedAt)>=72).length,lowStock:lowStock.length,outOfStock:lowStock.filter(item=>item.stock===0).length,openReturns:openReturns.length,newReturns:openReturns.filter(item=>item.status==="new").length,openMessages:openMessages.length,newMessages:openMessages.filter(item=>item.status==="new").length,draftNotifications:draftNotifications.length},
