...
export async function GET() {
  ...
  return Response.json({
    ...
    metrics:{activeOrders:activeOrders.length,newOrders:activeOrders.filter(order=>order.status==="new").length,preparingOrders:activeOrders.filter(order=>order.status==="preparing").length,lowStock:lowStock.length,outOfStock:lowStock.filter(item=>item.stock===0).length,openReturns:openReturns.length,newReturns:openReturns.filter(item=>item.status==="new").length,openMessages:openMessages.length,newMessages:openMessages.filter(item=>item.status==="new").length,draftNotifications:draftNotifications.length},
