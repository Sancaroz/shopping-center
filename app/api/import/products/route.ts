...
const validImageUrl=(value:string)=>!value||(value.startsWith("/")&&!value.startsWith("//"))||/^https:\/\//i.test(value);
...
export async function POST(request:Request){
  ...
  for(let index=0;index<rows.length;index++){
    ...
    if(!validImageUrl(imageUrl)||imageUrl.length>2000){errors.push(`${line}. satır: Görsel bağlantısı güvenli bir HTTPS veya site içi adres olmalı.`);skipped++;continue;}
    ...
    const values={nameTr,nameEn,slug,descriptionTr:String(source.descriptionTr??"").trim().slice(0,10_000),descriptionEn:String(source.descriptionEn??"").trim().slice(0,10_000),categoryId:category?.id??null,imageUrl,priceTr:number(String(source.priceTr??"")),priceGlobal:number(String(source.priceGlobal??"")),stock:Math.floor(number(String(source.stock??""))),marketTr:bool(String(source.marketTr??""),current?.marketTr??true),marketGlobal:bool(String(source.marketGlobal??""),current?.marketGlobal??false),active:requestedActive,updatedAt:new Date().toISOString()};
