...
export async function POST(request:Request){
  ...
  for(let index=0;index<rows.length;index++){
    ...
    const values={nameTr,nameEn,slug,descriptionTr:String(source.descriptionTr??"").trim().slice(0,10_000),descriptionEn:String(source.descriptionEn??"").trim().slice(0,10_000),categoryId:category?.id??null,imageUrl,priceTr,priceGlobal,stock,marketTr:bool(String(source.marketTr??""),current?.marketTr??true),marketGlobal:bool(String(source.marketGlobal??""),current?.marketGlobal??false),active:requestedActive,updatedAt:new Date().toISOString()};
