...
export async function POST(request:Request) { if (!(await getChatGPTUser())) return Response.json({ error:"Yetkisiz erişim" },{status:401}); const body=await request.json() as Record<string,unknown>; const productId=Number(body.productId); const sku=String(body.sku??"").trim(); const optionName=String(body.optionName??"").trim(); const optionValue=String(body.optionValue??"").trim(); if(!productId||!sku||!optionName||!optionValue)return Response.json({error:"Ürün, seçenek, değer ve ürün kodu zorunludur."},{status:400}); const [variant]=await getDb().insert(productVariants).values({productId,sku,optionName,optionValue,optionNameEn:String(body.optionNameEn??"").trim(),optionValueEn:String(body.optionValueEn??"").trim(),stock:Number(body.stock??0),priceAdjustment:Number(body.priceAdjustment??0)}).returning(); return Response.json({variant},{status:201}); }
