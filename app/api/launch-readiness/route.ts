...
export async function GET() {
  ...
  const checks=[
    ...
    {key:"email",label:"E-posta doğrulama",ready:integrations.email.adapterConnected,detail:integrations.email.adapterConnected?`${integrations.email.provider} gönderim adaptörü etkin.`:integrations.email.credentialsConfigured?`${integrations.email.provider||"E-posta sağlayıcısı"} bilgileri hazır; gönderim adaptörü ve teslimat testi bekleniyor.`:"Doğrulama bağlantıları güvenli biçimde hazırlanıyor; gerçek gönderim için sağlayıcı bağlantısı bekleniyor."},
