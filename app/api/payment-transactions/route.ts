...
export async function POST(request:Request){
  ...
  const transactionKey=`${provider.toLocaleLowerCase("en-US")}:${providerReference.toLocaleLowerCase("en-US")}:${kind}`;const ledgerSequence=status==="succeeded"&&reconciliationStatus==="matched"?successful.length+1:null;const transactionInsert=db.insert(paymentTransactions).values({orderId,transactionKey,kind,status,provider,providerReference,amount,currency:order.market==="TR"?"TRY":"EUR",source:"manual",reconciliationStatus,expectedAmount,ledgerSequence,note,occurredAt,actorEmail:user.email}).onConflictDoNothing().returning();
  ...
  if(!transaction)return Response.json({error:kind==="payment"&&status==="succeeded"?"Bu sipariş için başarılı tahsilat daha önce kaydedildi.":kind==="refund"&&status==="succeeded"?"Bu sırada başka bir iade kaydedildi. Güncel iade tutarını kontrol edip tekrar deneyin.":"Bu sağlayıcı referansı ve işlem türü daha önce kaydedildi."},{status:409,headers:noStore});
