...
export async function POST(request: Request) {
  ...
  if (!emailPattern.test(email) || email.length > 254) return Response.json({ error: "Geçerli bir e-posta adresi girin." }, { status: 400 });
...
export async function PATCH(request: Request) {
  ...
  if (!Number.isInteger(id) || typeof body.active !== "boolean") return Response.json({ error: "Geçersiz yönetici güncellemesi." }, { status: 400 });
  ...
  const displayName = typeof body.displayName === "string" ? body.displayName.trim().slice(0, 120) : member.displayName;
