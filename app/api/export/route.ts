...
export async function GET(request: Request) {
  ...
  if (type === "backup") {
    const [
      ...
      paymentWebhookReceiptRows,
      privacyRequestRows,
      fulfillmentChecklistRows,
      shipmentEventRows,
      inventoryOperationRows,
      inventoryOperationItemRows,
      ...
      db.select().from(paymentWebhookReceipts),
      db.select().from(privacyRequests),
      db.select().from(fulfillmentChecklists),
      db.select().from(shipmentEvents),
      db.select().from(inventoryOperations),
      db.select().from(inventoryOperationItems),
    ...
    const backup = await buildBackupEnvelope({
      ...
      paymentWebhookReceipts:paymentWebhookReceiptRows,
      privacyRequests:privacyRequestRows,
      fulfillmentChecklists:fulfillmentChecklistRows,
      shipmentEvents:shipmentEventRows,
      inventoryOperations:inventoryOperationRows,
      inventoryOperationItems:inventoryOperationItemRows,
