...
export async function GET(){
  ...
  const recent=active.slice(0,100).map(order=>{const lines=itemsByOrder.get(order.id)??[];const cogsAvailable=order.market==="TR"&&lines.every(item=>item.unitCostSnapshot>0);const cogs=cogsAvailable?lines.reduce((sum,item)=>sum+item.unitCostSnapshot*item.quantity,0):null;return{id:order.id,orderNumber:order.orderNumber,market:order.market,status:order.status,paymentStatus:order.paymentStatus,total:order.total,productRevenue:order.subtotal,cogs,grossProfit:cogs===null?null:order.subtotal-cogs,createdAt:order.createdAt};});
