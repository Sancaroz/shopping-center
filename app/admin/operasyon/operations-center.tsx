  ...
  return <main className="admin-shell operations-shell"><header className="admin-header"><div><p>GÜNLÜK YÖNETİM</p><h1>Operasyon merkezi</h1></div><div><button className="operations-refresh" onClick={load} disabled={refreshing}>{refreshing?"Yenileniyor…":"Verileri yenile"}</button><a href="/admin">Panele dön ↗</a></div></header>
    {!data?<section className="admin-card operations-loading">{message}</section>:<>
      ...
      <section className="operations-grid">
        ...
        <aside><section className="admin-card operations-links"><p className="section-kicker">HIZLI İŞLEMLER</p><h2>Yönetim alanları</h2><nav><a href="/admin">Siparişler <span>→</span></a><a href="/admin/tedarik">Tedarik takibi <span>→</span></a><a href="/admin/destek">Destek merkezi <span>→</span></a><a href="/admin/katalog-kalitesi">Katalog kalitesi <span>→</span></a><a href="/admin/teslimat-ayarlari">Teslimat kuralları <span>→</span></a><a href="/admin/iade-talepleri">İade ve iptal <span>→</span></a><a href="/admin/bildirimler">Bildirimler <span>→</span></a><a href="/admin/entegrasyonlar">Entegrasyonlar <span>→</span></a><a href="/admin/veri-guvenligi">Veri güvenliği <span>→</span></a><a href="/admin/islem-gecmisi">İşlem geçmişi <span>→</span></a><a href="/admin/yayina-hazirlik">Yayına hazırlık <span>→</span></a></nav></section><section className="admin-card stock-mini"><div className="list-title"><h2>Stok alarmı</h2><span>5 ve altı</span></div>{data.lowStock.slice(0,8).map(item=><a href={item.href} key={item.key}><span>{item.name}</span><b className={item.stock===0?"out":""}>{item.stock===0?"Tükendi":item.stock} →</b></a>)}</section></aside>
      </section>
