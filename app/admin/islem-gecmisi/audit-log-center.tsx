...
labels["data.export"]="Veri dışa aktarma";
...
function Changes({log}:{log:AuditLog}){
  ...
  const before=parse(log.beforeJson);const after=parse(log.afterJson);
...
export default function AuditLogCenter(){
  const[items,setItems]=useState<AuditLog[]>([]);const[filter,setFilter]=useState("all");const[message,setMessage]=useState("Yükleniyor…");const[refreshing,setRefreshing]=useState(false);const[loaded,setLoaded]=useState(false);
  const load=useCallback(async()=>{setRefreshing(true);try{const{response,data,error}=await requestJson<AuditPayload>("/api/audit-logs");if(response?.ok){setItems(data?.logs??[]);setLoaded(true);setMessage("");}else setMessage(data?.error??error??"İşlem geçmişi yüklenemedi. Lütfen tekrar deneyin.");}finally{setRefreshing(false);}},[]);
  useEffect(()=>{void load();},[load]);
  const visible=filter==="all"?items:items.filter(item=>item.entityType===filter);
  return <main className="admin-shell audit-shell">
    <header className="admin-header"><div><p>GÜVENLİK VE İZLENEBİLİRLİK</p><h1>Yönetim işlem geçmişi</h1></div><div><button onClick={load} disabled={refreshing}>{refreshing?"Yenileniyor…":"Kayıtları yenile"}</button><a href="/admin">Panele dön ↗</a><a href="/admin/yayina-hazirlik">Yayına hazırlık ↗</a></div></header>
    <section className="audit-intro"><div><b>{items.length}</b><span>Kayıtlı işlem</span></div><p>Bu kayıtlar salt okunurdur. Kritik yönetim değişikliklerinin kim tarafından ve ne zaman yapıldığını gösterir.</p></section>
    <nav className="audit-filters">{[["all","Tüm işlemler"],["product","Ürünler"],["category","Kategoriler"],["settings","Ayarlar"],["order","Siparişler"],["payment_transaction","Ödemeler"],["return_request","İade talepleri"],["notification","Bildirimler"],["contact_message","Destek"],["media","Medya"],["export","Dışa aktarma"]].map(([value,label])=><button className={filter===value?"active":""} onClick={()=>setFilter(value)} key={value}>{label}</button>)}</nav>
    ...
    <section className="audit-list">{visible.length?visible.map(log=><article className="admin-card" key={log.id}><header><div><span>{labels[log.action]??log.action}</span><h2>{log.summary}</h2></div><time>{new Date(log.createdAt).toLocaleString("tr-TR")}</time></header><p className="audit-actor"><b>{log.actorName}</b><span>{log.actorEmail}</span></p><Changes log={log}/></article>):loaded&&<div className="admin-card empty">Henüz kayıtlı işlem bulunmuyor.</div>}</section>
  </main>;
}
