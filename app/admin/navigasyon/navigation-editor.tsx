...
export default function NavigationEditor(){
  ...
  async function save(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);const response=await fetch("/api/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(values)});const data=await response.json();setMessage(response.ok?"Üst menü güncellendi.":data.error??"Menü kaydedilemedi.");setBusy(false);}
