...
export default function NavigationEditor(){
  ...
  useEffect(()=>{fetchSettings().then(({response,data})=>{if(!response.ok)throw new Error();setValues({...defaults,...data.settings});setMessage("");}).catch(()=>setMessage("Menü ayarları yüklenemedi."));},[]);
  async function save(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);setMessage("");try{const{response,data}=await updateSettings(values);setMessage(response.ok?"Üst menü güncellendi.":data.error??"Menü kaydedilemedi. Lütfen tekrar deneyin.");}catch{setMessage("Menü kaydedilemedi. Lütfen tekrar deneyin.");}finally{setBusy(false);}}
