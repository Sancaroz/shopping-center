...
type ShipmentPayload={events?:ShipmentEvent[];error?:string};
const statusOptions=[
...
export default function ShipmentManager({orderId,onChanged}:{orderId:number;onChanged:()=>Promise<void>}){
  ...
  const load=useCallback(async()=>{const{response,data,error}=await requestJson<ShipmentPayload>(`/api/shipment-events?orderId=${orderId}`);if(response?.ok)setEvents(data?.events??[]);else setMessage(data?.error??error??"Kargo hareketleri alınamadı. Lütfen tekrar deneyin.");},[orderId]);
  useEffect(()=>{void load()},[load]);
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);const form=event.currentTarget;try{const values=Object.fromEntries(new FormData(form));const occurredAt=new Date(String(values.occurredAt)).toISOString();const{response,data,error}=await requestJson<ShipmentPayload>("/api/shipment-events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({orderId,...values,occurredAt,visibleToCustomer:values.visibleToCustomer==="on"})});setMessage(response?.ok?"Kargo hareketi kaydedildi.":data?.error??error??"Hareket kaydedilemedi. Lütfen tekrar deneyin.");if(response?.ok){form.reset();await Promise.all([load(),onChanged()]);}}catch{setMessage("Kargo hareketi kaydedilemedi. Tarih bilgisini ve bağlantınızı kontrol edip tekrar deneyin.");}finally{setBusy(false);}}
