      ...
      {loading ? <p className="cart-empty">Çantanız yükleniyor…</p> : items.length === 0 ? <div className="cart-empty"><h2>Çantanız henüz boş.</h2><p>Seçkinizi oluşturmaya mağazadan başlayabilirsiniz.</p><a href="/#shop">Ürünleri keşfet →</a></div> : <div className="cart-layout">
        <div className="cart-lines">{items.map(item => {
          ...
          return <article className="cart-line" key={item.id}>
            ...
            <div className="cart-copy"><p>SEÇKİLİ ÜRÜN</p><h2><a href={`/urun/${encodeURIComponent(item.slug)}`}>{item.name}</a></h2>{item.optionValue && <span>{item.optionName}: {item.optionValue}</span>}<span>Adet: {item.quantity}</span><button onClick={() => remove(item.id)}>Kaldır</button></div>
            <strong>{money(unit * item.quantity)}</strong>
          </article>;
        })}</div>
        <aside className="cart-summary"><p>SİPARİŞ ÖZETİ</p><div><span>Ara toplam</span><strong>{money(total)}</strong></div><div><span>Teslimat</span><span>Ödeme adımında</span></div><hr/><div className="cart-total"><span>Toplam</span><strong>{money(total)}</strong></div><button disabled>Ödeme sistemi yakında</button><small>Ödeme bağlantısı, şirket ve ödeme sağlayıcısı hazır olduğunda güvenli biçimde eklenecek.</small></aside>
      </div>}
    </section>
  </main>;
}
