    ...
    <section className="checkout-shell">
      ...
      {!items.length ? <div className="checkout-empty"><p>{market==="GLOBAL"?"Add products to your bag to continue.":"Devam etmek için çantanıza ürün ekleyin."}</p><a href="/#shop">{market==="GLOBAL"?"Explore products":"Ürünleri keşfet"} →</a></div> : <div className="checkout-layout">
        <form className="checkout-form" onSubmit={submit}>
          ...
          <label>{market==="GLOBAL"?"Full name":"Ad soyad"}<input name="customerName" autoComplete="name" required/></label>
          <label>{market==="GLOBAL"?"Email":"E-posta"}<input name="email" type="email" autoComplete="email" required/></label>
          <label>{market==="GLOBAL"?"Phone":"Telefon"}<input name="phone" type="tel" autoComplete="tel" required/></label>
          <label>{market==="GLOBAL"?"Country":"Ülke"}<input name="country" autoComplete="country-name" defaultValue={market === "TR" ? "Türkiye" : ""} required/></label>
          ...
          <button className="wide" disabled={busy||!intakeOpen}>{!intakeOpen?(market==="GLOBAL"?"Order requests paused":"Sipariş alımı durduruldu"):busy ? (market==="GLOBAL"?"Saving…":"Kaydediliyor…") : (market==="GLOBAL"?"Create order request":"Sipariş talebini oluştur")}</button>
        </form>
