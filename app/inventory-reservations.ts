...
export async function releaseOrderReservation(db:Database,orderId:number,state="released",includeCommitted=false,options:ReleaseOptions={}){
  ...
  for(const line of lines){
    ...
    if(line.kind==="variant"){
      ...
      movementInserts.push(db.insert(inventoryMovements).select(db.select({id:sql<number|null>`NULL`,operationKey:sql<string>`${movementKey}`,productId:sql<number>`${line.productId}`,variantId:sql<number>`${line.id}`,orderId:sql<number>`${orderId}`,movementType:sql<string>`${"reservation_release"}`,quantityDelta:sql<number>`${line.quantity}`,previousStock:sql<number>`${productVariants.stock}-${line.quantity}`,nextStock:productVariants.stock,reason:sql<string>`${state==="expired"?"Süresi dolan rezervasyon serbest bırakıldı":order.reservationState==="committed"?"İptal edilen siparişin kesinleşmiş stoğu geri verildi":"Sipariş rezervasyonu serbest bırakıldı"}`,reference:sql<string>`${order.orderNumber}`,actorEmail:sql<string>`${"system"}`,createdAt:sql<string>`CURRENT_TIMESTAMP`}).from(productVariants).where(and(eq(productVariants.id,line.id),eq(productVariants.productId,line.productId),eq(productVariants.lastStockOperationKey,movementKey),releaseGuard()))).onConflictDoNothing());
    }else{
      ...
      movementInserts.push(db.insert(inventoryMovements).select(db.select({id:sql<number|null>`NULL`,operationKey:sql<string>`${movementKey}`,productId:sql<number>`${line.productId}`,variantId:sql<number|null>`NULL`,orderId:sql<number>`${orderId}`,movementType:sql<string>`${"reservation_release"}`,quantityDelta:sql<number>`${line.quantity}`,previousStock:sql<number>`${products.stock}-${line.quantity}`,nextStock:products.stock,reason:sql<string>`${state==="expired"?"Süresi dolan rezervasyon serbest bırakıldı":order.reservationState==="committed"?"İptal edilen siparişin kesinleşmiş stoğu geri verildi":"Sipariş rezervasyonu serbest bırakıldı"}`,reference:sql<string>`${order.orderNumber}`,actorEmail:sql<string>`${"system"}`,createdAt:sql<string>`CURRENT_TIMESTAMP`}).from(products).where(and(eq(products.id,line.id),eq(products.lastStockOperationKey,movementKey),releaseGuard()))).onConflictDoNothing());
    }
  }
