    ...
    <main>
      ...
      <footer>
        <div className="footer-top">
          <div><a className={`wordmark footer-logo${settings.brandLogoUrl?" image-wordmark":""}`} href="#top">{settings.brandLogoUrl?<img src={settings.brandLogoUrl} alt={`${settings.brandName} ${settings.brandSuffix}`}/>:<>{settings.brandName}<span>{settings.brandSuffix}</span></>}</a><p>{settings.footerTagline||"Beautiful things for considered living."}</p></div>
          ...
          <div className="newsletter"><h4>{settings.newsletterTitle||"Mektuplarımıza katılın"}</h4><p>{settings.newsletterCopy||"Yeni seçkiler ve ilham veren hikâyeler."}</p><form onSubmit={subscribe}><label className="newsletter-email"><span className="sr-only">E-posta adresi</span><input type="email" value={newsletterEmail} onChange={event=>setNewsletterEmail(event.target.value)} placeholder="E-posta adresiniz" required/><button aria-label="Kaydol">→</button></label><label className="newsletter-consent"><input type="checkbox" required/><span><a href="/politikalar#gizlilik">Gizlilik açıklamasını</a> okudum; seçki ve duyuruları almak istiyorum.</span></label>{newsletterMessage&&<small className="newsletter-message" role="status">{newsletterMessage}</small>}</form></div>
        </div>
        <div className="footer-bottom"><span>© 2026 {settings.brandName} {settings.brandSuffix}</span><span>{settings.footerLocation||"İstanbul · Dünya"}</span><span>{settings.instagramUrl?<a href={settings.instagramUrl} target="_blank" rel="noreferrer">Instagram</a>:"Instagram"} &nbsp; {settings.pinterestUrl?<a href={settings.pinterestUrl} target="_blank" rel="noreferrer">Pinterest</a>:"Pinterest"}</span></div>
      </footer>
